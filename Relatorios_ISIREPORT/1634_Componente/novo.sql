...
select
    ...
    case 
    when mpr.cod_mp is null then 'Pendente' 
    else 'Aprov' 
    end as Status_Corte, 
    case 
    when cmp.datacorte='1899.12.30' then ' ' 
    else substring (cmp.datacorte from 1 for 10 )
    end as Data_Corte,

    mpr.cod_mp as Material,
    cmp.Status_Comp as Status_Componentes,
    case when cmp.peso = cmp.pesosoldado1 then 'Soldado' else ' - ' end  as Solda
